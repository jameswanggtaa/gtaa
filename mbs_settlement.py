...
# Horizon used when as_of is within TBA_NEAR_CLASS_A_DAYS of the closest Class A.
# Example: 2026-08-14 is 1 day from 2026-08-13 Class A -> +30d = 2026-09-13 -> Sept.
TBA_HORIZON_DAYS = 30

# "Less than one week" from the closest Class A settle -> use (as_of + 30d) month.
TBA_NEAR_CLASS_A_DAYS = 7
...
def get_tba_horizon_date(
    as_of: Optional[date] = None,
    horizon_days: int = TBA_HORIZON_DAYS,
) -> date:
    """Return as_of + horizon_days (default 30). Example: 2026-08-14 -> 2026-09-13."""
    if as_of is None:
        as_of = date.today()
    return as_of + timedelta(days=horizon_days)


def get_tba_horizon_contract_month(
    as_of: Optional[date] = None,
    horizon_days: int = TBA_HORIZON_DAYS,
) -> str:
    """
    YYYY-MM of (as_of + horizon_days).

    Example: 2026-08-14 + 30 days = 2026-09-13 -> 2026-09 (Sept 2026 TBA).
    """
    return get_tba_horizon_date(as_of, horizon_days).strftime("%Y-%m")


def get_tba_horizon_prod_suffix(
    as_of: Optional[date] = None,
    horizon_days: int = TBA_HORIZON_DAYS,
) -> str:
    """Three-letter month for PROD-{MON} from the 30-day horizon month."""
    ym = get_tba_horizon_contract_month(as_of, horizon_days)
    return _MONTH_SUFFIX.get(ym[5:7], "APR")


def get_tba_horizon_settlement_date(
    as_of: Optional[date] = None,
    horizon_days: int = TBA_HORIZON_DAYS,
) -> str:
    """
    SIFMA Class A YYYY-MM-DD for the month of (as_of + horizon_days).

    Example: as_of=2026-08-14 -> horizon 2026-09-13 -> Sept 2026 Class A 2026-09-14.
    """
    ym = get_tba_horizon_contract_month(as_of, horizon_days)
    s = get_settlement_date(ym)
    if s:
        return s
    raise ValueError(
        f"No SIFMA Class A date for horizon contract month {ym}. "
        "Extend SIFMA_CLASS_A_SETTLEMENT in mbs_settlement.py."
    )


def get_closest_class_a_settlement_date(as_of: Optional[date] = None) -> str:
    """SIFMA Class A date nearest to as_of (past or future), YYYY-MM-DD."""
    if as_of is None:
        as_of = date.today()
    ...
    for settle_str in SIFMA_CLASS_A_SETTLEMENT.values():
        ...
        dist = abs((d - as_of).days)
        if best_dist is None or dist < best_dist:
            best_dist = dist
            best_str = settle_str
    if not best_str:
        raise ValueError("SIFMA_CLASS_A_SETTLEMENT is empty.")
    return best_str


def days_from_closest_class_a(as_of: Optional[date] = None) -> int:
    """Calendar days between as_of and the closest Class A settle date."""
    if as_of is None:
        as_of = date.today()
    closest = date.fromisoformat(get_closest_class_a_settlement_date(as_of))
    return abs((closest - as_of).days)


def tba_use_horizon_settle_month(as_of: Optional[date] = None) -> bool:
    """True if as_of is less than one week from the closest Class A date."""
    return days_from_closest_class_a(as_of) < TBA_NEAR_CLASS_A_DAYS


def get_tba_settle_contract_month(as_of: Optional[date] = None) -> str:
    """
    TBA contract month (YYYY-MM).

    If as_of is less than one week from the closest Class A date, use the month
    of (as_of + 30 days). Otherwise use the next Class A month on or after as_of.
    """
    if as_of is None:
        as_of = date.today()
    if tba_use_horizon_settle_month(as_of):
        return get_tba_horizon_contract_month(as_of)
    return get_next_settlement_date(as_of)[:7]


def get_tba_settle_prod_suffix(as_of: Optional[date] = None) -> str:
    """Three-letter month for PROD-{MON} from get_tba_settle_contract_month."""
    ym = get_tba_settle_contract_month(as_of)
    return _MONTH_SUFFIX.get(ym[5:7], "APR")


def get_tba_settle_date(as_of: Optional[date] = None) -> str:
    """
    SIFMA Class A YYYY-MM-DD for get_tba_settle_contract_month.

    Near a Class A date (< 7 days): Class A of (as_of + 30d) month.
    Else: next Class A on or after as_of.
    """
    ym = get_tba_settle_contract_month(as_of)
    s = get_settlement_date(ym)
    if s:
        return s
    raise ValueError(
        f"No SIFMA Class A date for TBA settle month {ym}. "
        "Extend SIFMA_CLASS_A_SETTLEMENT in mbs_settlement.py."
    )
...
def get_previous_business_day(as_of: Optional[date] = None) -> date:
    """Last US business day strictly before as_of (weekends excluded; no holiday calendar)."""
    if as_of is None:
        as_of = date.today()
    return get_last_business_day(as_of - timedelta(days=1))


def get_previous_business_day_iso(as_of: Optional[date] = None) -> str:
    """Previous business day as YYYY-MM-DD (used when today's SWAP_RFR curve is unpublished)."""
    return get_previous_business_day(as_of).strftime("%Y-%m-%d")


# Use in Yieldbook API:
# - pricing_date = get_previous_business_day_iso()  -> prior business day SWAP_RFR
# - settlement_date = get_tba_settle_date() -> Class A of next month if within 7d of
#   closest Class A, else next Class A on/after as_of
# - PROD-{MON} = get_tba_settle_prod_suffix(as_of)
...
if __name__ == "__main__":
    ...
    print("As of 2026-08-14 (1d after Aug Class A -> +30d month):")
    t3 = date(2026, 8, 14)
    print("  Closest Class A:", get_closest_class_a_settlement_date(t3), "days=", days_from_closest_class_a(t3))
    print("  TBA contract month:", get_tba_settle_contract_month(t3), get_tba_settle_prod_suffix(t3))
    print("  TBA settle (Class A):", get_tba_settle_date(t3))
    print("As of 2026-08-01 (12d before Aug Class A -> next Class A):")
    t4 = date(2026, 8, 1)
    print("  Closest Class A:", get_closest_class_a_settlement_date(t4), "days=", days_from_closest_class_a(t4))
    print("  TBA contract month:", get_tba_settle_contract_month(t4), get_tba_settle_prod_suffix(t4))
    print("  TBA settle (Class A):", get_tba_settle_date(t4))
    print("As of 2026-08-07 (6d before Aug Class A -> +30d month):")
    t5 = date(2026, 8, 7)
    print("  Closest Class A:", get_closest_class_a_settlement_date(t5), "days=", days_from_closest_class_a(t5))
    print("  TBA contract month:", get_tba_settle_contract_month(t5), get_tba_settle_prod_suffix(t5))
    print("  TBA settle (Class A):", get_tba_settle_date(t5))
